...
C A wrapper to use ESSL lib
      ...
      if (job .eq. 0) then
         ...
         transa = 'T'
C  dgetrs can actually handle 3 cases, but dgesl only 2.
      endif
      call dgetrs (transa, n, nrhs, a, lda, ipvt, b, ldb, info)
      if (info .ne. 0) then
         write (*,*) 'Warning: matrix from dgetrs is singular'
      endif
      do i = 1, n
         b(i) = bv(i, nrhs)
      enddo
