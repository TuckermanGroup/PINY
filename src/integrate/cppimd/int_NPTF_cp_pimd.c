...
/*cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc*/
/*==========================================================================*/
void int_NPTF_cp_pimd(CLASS *class,BONDED *bonded,
...
/*    double kinet_cp;*/
    ...
    double *class_par_rahman_roll_mtx = general_data->par_rahman.roll_mtx;
    double *class_par_rahman_roll_mtv = general_data->par_rahman.roll_mtv;
  ...
  if(myid_state==0){
    ...
    if((cp->cptherm_info.num_c_nhc > 0) && (cp->cptherm_info.len_c_nhc > 0)) {
      ...
      set_yosh(cp->cptherm_info.nyosh_c_nhc,cp->cptherm_info.dti_nhc ,
               ...
               cp->cptherm_info.wdti4,
               cp->cptherm_info.wdti8,
...
/* II) Evolve classical part of system  from t=0 to dt/2                    */
    ...
    if(myid_state==0){
      int_0_to_dt2_nptf_pimd(class,bonded,general_data,ir_tra,ir_tor,
                                                       ir_ter,ir_pimd,dt);
    }/*endif*/
...
/* IV) Evolve classical part of system  from t=dt/2 to dt                   */
    ...
    if(myid_state==0){
      int_dt2_to_dt_nptf_pimd(class,bonded,general_data,ir_tra,ir_tor,
                                                    ir_ter,ir_pimd,dt); 
    }
