...
/*             Local variable declarations                                */
    ...
    int iii;
    int i,j,ipart,inhc,ichain,ktemp;     /* Num: for loop counters */
...
/*--------------------------------------------------------------------------*/
/*  3) Evolve the vgmat with NHC                                            */

        arg = -therm_wdti8[iyosh]*baro_v_vol_nhc[1]*wght;
        aa = exp(arg);
        aa2 = aa*aa;
        (*par_rahman_roll_scg) =
              (*par_rahman_roll_scg)*aa2;
...
/*--------------------------------------------------------------------------*/
/* 4) Evolve the particle velocities with NHC                               */

         for(inhc=mytherm_start;inhc<=mytherm_end;inhc++){
           arg = -therm_wdti2[iyosh]*therm_v_nhc[1][inhc]*wght;
